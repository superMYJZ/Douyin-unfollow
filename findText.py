...
try:
    ...
    print('模块导入错误,请使用pip安装,pytesseract依赖以下之你个的 而得的库：')
    ...
    for box in region:
        ...
        if box[0][0] * box[1][1] * box[2][0] * box[0][1] != 0:
            ...
            if len(res) != 0:
                a={}
                a['name'] = ''.join(res);
                a['top_high'] = str(box[1][1]);
                a['top_left'] = str(box[1][0]);
                a['bottom_high'] = str(box[3][1]);
                a['bottom_right'] = str(box[3][0]);
                result.append(a)

                if a['name']=='已关注':
                    center_high=(box[0][1]+box[1][1])/2
                    needreturn.append(center_high)
